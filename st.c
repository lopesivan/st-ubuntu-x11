...
size_t ttyread(void)
{
    ...
    static int buflen = 0;
    /* read() retorna ssize_t e espera size_t —
       em buffers grandes (BUFSIZ = 8192 tipicamente) não é problema,
       mas o cast implícito pode gerar warning com -Wsign-conversion */
    int ret, written;
...
void externalpipe(const Arg* arg)
{
    ...
    close(to[0]);
    /* o fork() acontece e o filho execvp antes do pai terminar de
       escrever no pipe — isso é intencional e está ok pelo design,
       mas o comentário "in case child exists early" tem um typo:
       "exists" deveria ser "exits" */
...
void copyurl(const Arg* arg)
{
    ...
    */
    while (passes < term.bot + 2) {
        ...
        for (col = 0; col < colend && i < term.col; col++) {
            if (term.line[row][col].u <= 127) {
                linestr[i++] = (char)term.line[row][col].u;
            }
        }
...
{
    char* found;
    size_t i, flen[len];  /* VLA — ou malloc se preferir C89 */
    
    for (i = 0; i < len; i++) {
        flen[i] = strlen(find[i]);
    }
    
    for (found = str + strlen(str) - 1; found >= str; found--)
        for (i = 0; i < len; i++)
            if (strncmp(found, find[i], flen[i]) == 0) {
                return found;
            }
